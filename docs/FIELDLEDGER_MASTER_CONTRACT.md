...
FieldLedger is an offline-first 1099 field-work tracking app for job tickets, pay periods, receipts, expenses, and rough tax planning.

The app is designed for workers who need to capture handwritten job ticket information, manually review it, calculate earnings, track expenses, and export pay-period records.

## 2. Non-Negotiable Rules

### 2.1 Offline-First Rule

FieldLedger must work without requiring an internet connection for core use.

Core offline functions include:

- Creating a pay period
- Adding job ticket entries
- Taking or attaching ticket photos
- Manually entering job pay details
- Adding receipt entries
- Taking or attaching receipt photos
...
The MVP must not depend on paid APIs, paid AI services, paid OCR services, paid databases, or paid hosting.
...
The user must be able to edit all job, receipt, expense, and pay values before saving.

### 2.5 Tax Disclaimer Rule

Tax estimates are for planning only.

FieldLedger must not present rough tax estimates as tax advice, tax filing guidance, or guaranteed tax liability.
...
```text
buckingTotal = hoursWorked × hourlyRate
...
torqueTurnTotal = baseJobPay + (extraHours × hourlyRate)
3.3 Hourly Rate

The default hourly rate is user-controlled.

The current starting value is:

$28/hour

Each saved job must store the hourly rate used at the time of saving so old jobs do not change if the user changes the rate later.

4. Locked Expense Rules

Expenses belong inside a pay period.

Receipt entries must support:
...
The app must display that this is only a rough planning estimate.

7. Future Contract Files

FieldLedger must be governed by separate contracts as the app grows:
