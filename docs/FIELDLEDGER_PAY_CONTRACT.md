...
The current default hourly rate is:
...
Changing the user’s hourly rate later must not change old saved job totals.
...
Blank numeric values should be treated as zero while editing, but required values must be checked before saving.
